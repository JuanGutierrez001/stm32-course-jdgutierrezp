...
void separar_digitos(void);//FUNCION PARA LA ASIGNACION  DE LAS SEPARACION DE LOS DIGITOS EN MILLARES, CENTENAS , DECENAS
void timer2_display(void);//FUNCION PARA EL REFRESCO DEL DISPLAY
void pintar_numero_7seg(uint8_t numero);//FUNCION PARA ACTIVACION DE PINES DEL 7 SEGMENTOS
...
//ISR PARA EL EXTI0
// VERIFICA QUE EL PR ESTA LEVATADO
//
...
void exti_1(void){
	...
	EXTI->RTSR |= EXTI_RTSR_TR1;//CONFIGURACION DE FLANCO SUBIDA--ESCRITURA DE 1
...
void pintar_numero_7seg(uint8_t numero) {
    // 1. APAGA TODOS LOS SEGMENTOS PRIMERO (Poner en 1) |11|
    GPIOC->ODR |= (GPIO_ODR_OD9 | GPIO_ODR_OD11 | GPIO_ODR_OD12 | GPIO_ODR_OD10 | GPIO_ODR_OD8);
    GPIOB->ODR |= (GPIO_ODR_OD8);
    GPIOD->ODR |= (GPIO_ODR_OD2);

    // 2. ENCIENDE LOS PINES NECESARIOS (SE SABE QUE PÓR SER DE ANODO COMUN SE ENCIENDEN CON UN 0 LOGICO)
    switch(numero) {
...
void timer2_display(void){		//TIMER REFRESCO DISPLAY
	...
	TIM2->ARR = 4 - 1;     // INTERRUPCIONES CADA 4 ms (REFRESCO DEL DISPLAY)
